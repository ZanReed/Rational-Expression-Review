...
function renderProblems() {
  ...
  builderState.problems.forEach((p, idx) => {
    ...
    stemArea.id = 'stemArea_' + p.id;
    stemArea.rows = 3;
    stemArea.value = p.stem || '';
    stemArea.placeholder = 'Type the problem text. Insert {{blank:N}} where students should answer.';
    stemArea.oninput = () => {
      // Save keystrokes to state without re-rendering — preserves focus and
      // lets the user type freely (including newlines).
      const live = builderState.problems.find(x => x.id === p.id);
      if (live) live.stem = stemArea.value;
      saveDraft();
    };
    stemArea.onblur = () => {
      // On blur: reconcile blanks-to-stem (renumber tokens, trim/grow blank
      // config). This re-renders, which is fine since focus has already left.
      _syncBlanksToStem(p.id);
    };
    card.appendChild(stemArea);

    // ----- Insert-blank shortcut buttons
    // Note: don't capture `stemArea` in the click handlers — re-renders detach
    // it. Look up the live textarea by id at click time instead.
    ...
    fillBtn.onclick = () => {
      const ta = document.getElementById('stemArea_' + p.id);
      if (ta) _insertBlankAtCursor(p.id, ta, 'fill_in');
    };
    ...
    dropBtn.onclick = () => {
      const ta = document.getElementById('stemArea_' + p.id);
      if (ta) _insertBlankAtCursor(p.id, ta, 'dropdown');
    };
...
// can find any \( \) or $$ $$ math delimiters inside. Newlines become <br>.
function _compileStemText(s) {
  // Escape would break math delimiters like \( and \[, so we only handle <br>.
  // The teacher controls stem content; if they want literal HTML they can use it.
  return '<span class="stem-text">' + String(s).replace(/\n/g, '<br>') + '</span>';
}
